import pyautogui
import os
import time
from pynput.mouse import Controller

# Safety feature: stop the bot if the mouse moves quickly to a screen corner
pyautogui.FAILSAFE = True

# Create a pynput mouse controller (for scrolling)
mouse_ctrl = Controller()

# The image file names you provided are listed here in the correct order
image_sequence = [
    '(1)attack!.png',             # step 1
    '(2)Find a Match 1700.png',   # step 2
    '(3)Attack!.png',             # step 3 (triggers zoom-out and attack macro)
    '(4)Return Home.png'          # step 4
]

print("--- Clash of Clans Automated Bot (Single Run) Started ---")
print("Checking images in sequence...\n")

current_step = 0  # starting step

# REMOVED: while True loop - now runs only once through all steps
while current_step < len(image_sequence):
    target_image = image_sequence[current_step]
    print(f"Searching: {target_image} (Step {current_step + 1}/{len(image_sequence)})", end="\r")

    try:
        # Check whether the target step image is present on the screen
        location = pyautogui.locateOnScreen(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'Images', target_image), confidence=0.8)
        
        if location is not None:
            print(f"\n[FOUND] {target_image} image found on the screen!")
            center_point = pyautogui.center(location)
            
            # Move the mouse to the target location and click it
            pyautogui.click(center_point)
            print(f"[CLICK] Click position: {center_point}")
            
            # Special handling for step 3 (Attack!) after it is clicked
            if target_image == '(3)Attack!.png':
                print("[INFO] Attack started. Preparing to zoom out the map...")
                time.sleep(1) # wait 1 second for the attack screen to load
                
                # 1. Find the exact center coordinates of the screen
                screen_width, screen_height = pyautogui.size()
                center_x = screen_width // 2
                center_y = screen_height // 2
                
                # 2. Move the mouse to the center of the screen
                pyautogui.moveTo(center_x, center_y, duration=0.3)
                print(f"[MOVE] Mouse moved to center: ({center_x}, {center_y})")
                time.sleep(0.5)
                
                # 3. Perform a long scroll out (zoom out)
                print("[SCROLL] Performing extended zoom out...")
                for _ in range(15):
                    mouse_ctrl.scroll(0, -1)
                    time.sleep(0.05)
                print("[SUCCESS] Zoom out complete!")
                time.sleep(1)
                
                # 4. Start the provided attack (troop deployment) macro
                print("\n🚀 [MACRO] Starting troop deployment macro...")
                
                # 10 second sleep for load to attack base 
                time.sleep(10)                 
                #Electro Dragons
                #Electro Dragon Select   
                pyautogui.click(263, 1119)
                time.sleep(4.14)
                #1Electro Dragon Drop
                pyautogui.click(639, 902)
                time.sleep(0.30)
                #2Electro Dragon Drop
                pyautogui.click(552, 841)
                time.sleep(0.25)
                #3Electro Dragon Drop
                pyautogui.click(478, 760)
                time.sleep(0.25)
                #4Electro Dragon Drop
                pyautogui.click(397, 706)
                time.sleep(0.25)
                #5Electro Dragon Drop
                pyautogui.click(321, 639)
                time.sleep(0.20)
                #6Electro Dragon Drop
                pyautogui.click(220, 567)
                time.sleep(0.20)
                #7Electro Dragon Drop
                pyautogui.click(310, 472)
                time.sleep(0.25)
                #8Electro Dragon Drop
                pyautogui.click(418, 387)
                time.sleep(0.30)
                #9Electro Dragon Drop
                pyautogui.click(508, 315)
                time.sleep(0.20)
                #10Electro Dragon Drop
                pyautogui.click(585, 261)
                time.sleep(0.20)
                #11Electro Dragon Drop
                pyautogui.click(674, 203)
                time.sleep(0.20)

                #Balloons
                #Balloon Select
                pyautogui.click(354, 1113)
                time.sleep(1.34)
                #1Balloon Drop
                pyautogui.click(259, 609)
                time.sleep(0.30)
                #2Balloon Drop
                pyautogui.click(359, 671)
                time.sleep(0.25)
                #3Balloon Drop
                pyautogui.click(420, 723)
                time.sleep(0.25)

                #Archers
                #Archer Select
                pyautogui.click(549, 1097)
                time.sleep(0.55)
                #1Archer Drop
                pyautogui.click(214, 549)
                time.sleep(0.30)

                #Vehicles
                #Stone Slammer Select
                pyautogui.click(681, 1087)
                time.sleep(0.63)
                #1Stone Slammer Drop
                pyautogui.click(211, 542)
                time.sleep(0.30)
                

                # (1) Barbarian King
                pyautogui.click(848, 1097)
                time.sleep(0.82)
                pyautogui.click(206, 544)
                time.sleep(1.32)

                # (2) Archer Queen
                pyautogui.click(951, 1076)
                time.sleep(0.84)
                pyautogui.click(459, 341)
                time.sleep(0.99)

                # (3) Grand Warden
                pyautogui.click(1099, 1063)
                time.sleep(0.84)
                pyautogui.click(442, 730)
                time.sleep(1.14)

                # (4) Royal Champion
                pyautogui.click(1239, 1095)
                time.sleep(0.85)
                pyautogui.click(205, 552)
                time.sleep(0.98)

                #Rage Spell Select
                pyautogui.click(1371, 1121)
                time.sleep(0.82)
                #1Rage Spell Drop
                pyautogui.click(880, 689)
                time.sleep(1.18)
                #2Rage Spell Drop
                pyautogui.click(776, 594)
                time.sleep(0.25)
                #3Rage Spell Drop
                pyautogui.click(772, 492)
                time.sleep(0.25)
                #4Rage Spell Drop
                pyautogui.click(906, 448)
                time.sleep(0.25)
                #5Rage Spell Drop
                pyautogui.click(899, 545)
                time.sleep(0.30)

                #Freeze Spell Select
                pyautogui.click(1538, 1111)
                time.sleep(1.11)
                #1Freeze Spell Drop
                pyautogui.click(980, 555)
                time.sleep(1.53)

                #Barbarian King Ability
                pyautogui.click(835, 1105)
                time.sleep(1.26)
                #Archer Queen Ability
                pyautogui.click(953, 1093)
                time.sleep(0.83)
                #Grand Warden Ability
                pyautogui.click(1111, 1098)
                time.sleep(0.50)
                #Royal Champion Ability
                pyautogui.click(1208, 1101)
                time.sleep(0.55)
                
                print("[SUCCESS] Attack Macro finished. Now waiting for the Return Home Button...\n")
            
            # Proceed to the next step after successful click and macro completion
            current_step += 1
            time.sleep(2)
            
    except pyautogui.ImageNotFoundException:
        pass
    except Exception as e:
        print(f"\n[ERROR] Unexpected error occurred: {e}")
        
    time.sleep(0.5)

# Program ends automatically after completing all 4 steps
print("\n==========================================")
print("[SUCCESS] One Attack successfully completed!")
print("Program will now exit.")
print("==========================================")